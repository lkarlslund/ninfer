...
int run(const Options& options) {
    ...
    auto* family =
        std::get_if<std::unique_ptr<ninfer::runtime::ModelInstance>>(&constructed.instance);
    if (!family) {
        throw std::invalid_argument("this round benchmark requires a Qwen3.5-family artifact");
    }
    auto& frontend = (*family)->frontend;
    auto& program  = (*family)->program;
