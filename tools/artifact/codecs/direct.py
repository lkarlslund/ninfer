"""Exact BF16, FP32, INT32, and raw E4M3FN words in contiguous little-endian layout."""
...
def encode_direct(tensor: torch.Tensor, format: str | DirectFormat) -> bytes:
    ...
    if not isinstance(spec, DirectFormat):
        raise ValueError("direct encoding requires a direct scalar format")
...
def decode_direct(
    ...
    spec = _format(format)
    if not isinstance(spec, DirectFormat):
        raise ValueError("direct decoding requires a direct scalar format")
    dims = _shape(shape)
    if len(dims) > 16:
