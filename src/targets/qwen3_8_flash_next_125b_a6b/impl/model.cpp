...
Tensor tensor(const artifact::MaterializedArtifact& artifact, artifact::ObjectHandle handle,
              std::initializer_list<std::int32_t> shape,
              artifact::NumericFormat format = artifact::NumericFormat::BF16) {
    return artifact::materialized_tensor(artifact, handle, format, shape);
}

Weight bf16(const artifact::MaterializedArtifact& artifact, artifact::ObjectHandle handle, int rows,
            int columns) {
    return artifact::materialized_weight(artifact, handle, artifact::NumericFormat::BF16, rows,
                                         columns);
}
...
ops::FlashNextExpertBank nvfp4_bank(const artifact::MaterializedArtifact& artifact,
                                    ...
                                    artifact::ObjectHandle input_divisors, int rows, int columns) {
    const std::uint64_t shape[] = {512, static_cast<std::uint64_t>(rows),
                                   static_cast<std::uint64_t>(columns)};
    const artifact::ExpertBlockScaleGeometry geometry =
        artifact::expert_block_scale_geometry(artifact::NumericFormat::NVFP4, shape);
    const auto* base = static_cast<const std::byte*>(artifact.device_data(handle));
    return {
        .codes  = base,
        .scales = base + geometry.scale_plane_offset,
        .weight_scale_divisors =
            reinterpret_cast<const float*>(base + geometry.divisor_plane_offset),
        .input_scale_divisors = static_cast<const float*>(artifact.device_data(input_divisors)),
        .qtype                = QType::NVFP4,
        .experts              = 512,
        .rows                 = rows,
        .columns              = columns,
...
ops::FlashNextExpertBank bf16_bank(const artifact::MaterializedArtifact& artifact,
                                   artifact::ObjectHandle handle, int rows, int columns) {
    return {
        .codes   = artifact.device_data(handle),
...
qwen3_8_flash_next::VisionWeights vision(const VisionPlan& plan,
    ...
    common.patch_embedding = artifact::materialized_weight(artifact, plan.patch_embedding,
                                                           NumericFormat::Q6G64_F16S, 1152, 1536);
    ...
    for (std::size_t layer = 0; layer < common.layers.size(); ++layer) {
        ...
        target.qkv = artifact::materialized_weight(artifact, source.qkv, NumericFormat::Q4G64_F16S,
                                                   3456, 1152);
        target.qkv_bias    = tensor(artifact, source.qkv_bias, {3456});
        target.output      = artifact::materialized_weight(artifact, source.output,
                                                           NumericFormat::Q5G64_F16S, 1152, 1152);
        target.output_bias = tensor(artifact, source.output_bias, {1152});
        target.fc1 = artifact::materialized_weight(artifact, source.fc1, NumericFormat::Q4G64_F16S,
                                                   4304, 1152);
        target.fc1_bias = tensor(artifact, source.fc1_bias, {4304});
        target.fc2 = artifact::materialized_weight(artifact, source.fc2, NumericFormat::Q5G64_F16S,
                                                   1152, 4304);
    ...
    common.merger_fc1      = artifact::materialized_weight(artifact, plan.merger_fc1,
                                                           NumericFormat::W8G32_F16S, 4608, 4608);
    ...
    out.merger_fc2      = artifact::materialized_weight(artifact, plan.merger_fc2,
                                                        NumericFormat::W8G32_F16S, 2560, 4608);
...
take_frontend(artifact::MaterializedArtifact& artifact,
    ...
    auto take = [&](std::size_t index) {
        const auto bytes = artifact.take_resource_bytes(plan[index]);
...
LoadedModelData::LoadedModelData(BindingPlan plan, artifact::MaterializedArtifact materialized)
    : backing(std::move(materialized)), frontend(take_frontend(backing, plan.frontend)) {
    ...
    if (plan.features.optimized_proposal()) {
        ...
        proposal.head =
            artifact::materialized_weight(backing, plan.optimized_proposal_head,
                                          artifact::NumericFormat::Q4G64_F16S, 147456, 2560);
    ...
    runtime.ple_table = backing.file_backed_bytes(plan.text_layers[1].ple.embedding_table);
}
