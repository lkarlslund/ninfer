...
VisionBackbonePlan bind_vision_backbone(artifact::Binder& binder,
                                        artifact::TensorPlacement placement, bool q8_weights) {
    using artifact::NumericFormat;
    const auto quant = [&](NumericFormat native) {
        return q8_weights ? NumericFormat::W8G32_F16S : native;
    };
    ...
    out.patch_embedding = bind("vision/patch_embedding", quant(NumericFormat::Q6G64_F16S),
    ...
    for (std::size_t layer = 0; layer < out.layers.size(); ++layer) {
        ...
        target.qkv               = bind(prefix + "attention/qkv", quant(NumericFormat::Q4G64_F16S),
        ...
        target.output            = bind(prefix + "attention/output", quant(NumericFormat::Q5G64_F16S),
        ...
        target.fc1               = bind(prefix + "mlp/fc1", quant(NumericFormat::Q4G64_F16S),
        ...
        target.fc2               = bind(prefix + "mlp/fc2", quant(NumericFormat::Q5G64_F16S),
...
VisionCommonWeights materialize_vision_common(const artifact::MaterializedArtifact& materialized,
    ...
    out.patch_embedding = artifact::materialized_weight(
        materialized, backbone.patch_embedding, quant(NumericFormat::Q6G64_F16S),
        VisionBackboneConfig::hidden, VisionBackboneConfig::patch_dim);
    out.patch_embedding_bias =
    ...
    for (std::size_t layer = 0; layer < out.layers.size(); ++layer) {
        ...
        target.qkv                    = artifact::materialized_weight(
            materialized, source.qkv, quant(NumericFormat::Q4G64_F16S), 3 * VisionBackboneConfig::hidden,
            VisionBackboneConfig::hidden);
        ...
        target.output = artifact::materialized_weight(
            materialized, source.output, quant(NumericFormat::Q5G64_F16S), VisionBackboneConfig::hidden,
            VisionBackboneConfig::hidden);
        ...
        target.fc1 = artifact::materialized_weight(
            materialized, source.fc1, quant(NumericFormat::Q4G64_F16S), VisionBackboneConfig::intermediate,
            VisionBackboneConfig::hidden);
        target.fc1_bias =
            artifact::materialized_tensor(materialized, source.fc1_bias, NumericFormat::BF16,
                                          {VisionBackboneConfig::intermediate});
        target.fc2 = artifact::materialized_weight(
            materialized, source.fc2, quant(NumericFormat::Q5G64_F16S), VisionBackboneConfig::hidden,
            VisionBackboneConfig::intermediate);
