...
PersistentLayout persistent_layout(const SequencePlanImpl& plan) {
    ...
    out.decoder = qwen3_8_flash_next::plan_decoder_state(
        builder, qwen3_8_flash_next::DecoderStateSpec{
                     ...
                     .auxiliary_planes =
                         [] {
                             if constexpr (requires { Variant::flash_next; }) {
                                 if constexpr (Variant::flash_next) {
                                     return std::vector<KVPlaneGeometry>{{DType::BF16, 128, 1, 256},
                                                                         {DType::I32, 3, 1, 256}};
                                 }
                             }
                             return std::vector<KVPlaneGeometry>{};
                         }(),
                 });
    qwen3_8_flash_next::StateImageSpec state_image_spec{
        .linear =
            {
        ...
        .mtp_hidden = [&]() -> std::optional<std::int32_t> {
            if constexpr (Variant::flash_next) {
                if (plan.features.mtp()) { return 4 * TextConfig::hidden; }
            }
            return std::nullopt;
        }(),
        .ple = []() -> std::optional<qwen3_8_flash_next::PleStateSpec> {
            if constexpr (Variant::flash_next) {
                return qwen3_8_flash_next::PleStateSpec{.channels = 4 * TextConfig::hidden,
                                                        .width    = 9};
            }
            return std::nullopt;
        }(),
    ...
    if constexpr (Variant::supports_dflash) {
        if (plan.features.dflash()) {
            state_image_spec.dflash_local = qwen3_8_flash_next::DFlashLocalStateSpec{
    ...
    if constexpr (Variant::supports_dflash) {
        if (plan.features.dflash()) {
            ...
            dflash.full = qwen3_8_flash_next::PagedKVCacheLayout{
    ...
    out.round = qwen3_8_flash_next::begin_round_state_layout(
        builder, qwen3_8_flash_next::RoundStateSpec{.hidden = TextConfig::hidden,
                                                    .mtp_hidden =
                                                        [&] {
                                                            if constexpr (Variant::flash_next) {
                                                                return plan.features.mtp()
                                                                           ? 4 * TextConfig::hidden
                                                                           : 0;
                                                            }
                                                            return 0;
    ...
    if constexpr (Variant::flash_next) {
        if (plan.features.mtp()) {
            out.mtp_prefill_hidden =
                add_tensor(builder, DType::BF16, {4 * TextConfig::hidden, effective_prefill_chunk},
        ...
        out.flash_decode_ple = add_tensor(
            ...
            {TextConfig::hidden,
             ...
             static_cast<std::int32_t>(plan.max_concurrency)},
            "Flash-Next decode PLE embeddings");
        if (plan.speculative_backend != SpeculativeBackend::None) {
            out.flash_ple_records = add_tensor(builder, DType::BF16,
                                               {4 * TextConfig::hidden,
                                                static_cast<std::int32_t>(plan.draft_window + 1U),
                                                static_cast<std::int32_t>(plan.max_concurrency)},
    ...
    qwen3_8_flash_next::complete_round_state_layout(builder, out.round);
...
WorkspacePlan build_workspace_plan(const SequencePlanImpl& plan) {
    ...
    const auto attention_stage = [&](WorkspaceLayoutBuilder& layout, std::int32_t first,
                                     std::int32_t last, qwen3_8_flash_next::TextPhase phase,
    ...
    const auto gdn_stage = [&](WorkspaceLayoutBuilder& layout, std::int32_t first,
                               std::int32_t last, qwen3_8_flash_next::TextPhase phase,
                               GdnWorkspacePath path, std::int32_t batch_size,
    ...
    const auto target_body = [&](WorkspaceLayoutBuilder& layout, std::int32_t first,
        ...
        if constexpr (Variant::flash_next) {
            ...
            matrix(layout, DType::BF16, 4, last);
            scratch(layout, ops::hyperconnection_mix_workspace_capacity_bytes(last, true));
            {
                auto ple = layout.scope();
                matrix(layout, DType::BF16, 10240, last);
    ...
    const auto proposal_scratch = [&](WorkspaceLayoutBuilder& layout, std::int32_t columns) {
        if (plan.proposal_head == ProposalHead::Optimized) {
            ...
            if constexpr (Variant::flash_next) {
                constexpr std::int32_t kCandidateRows =
                    8 * ((Variant::draft_head_rows + 511) / 512);
                matrix(layout, DType::I32, kCandidateRows, columns);
                matrix(layout, DType::FP32, kCandidateRows, columns);
            }
        }
    };
    ...
    const auto mtp_full_core = [&](WorkspaceLayoutBuilder& layout, std::int32_t tokens,
        ...
        if constexpr (Variant::flash_next) {
            matrix(layout, DType::BF16, TextConfig::hidden, tokens);     // embedding
            matrix(layout, DType::BF16, TextConfig::hidden, tokens);     // normalized embedding
            matrix(layout, DType::BF16, TextConfig::hidden, tokens);     // projected embedding
            matrix(layout, DType::BF16, 4 * TextConfig::hidden, tokens); // normalized predictor
            matrix(layout, DType::BF16, 4 * TextConfig::hidden, tokens); // hyper state
            matrix(layout, DType::BF16, TextConfig::hidden, tokens);     // block input
            matrix(layout, DType::BF16, TextConfig::hidden, tokens);     // block output
            matrix(layout, DType::BF16, 4, tokens);                      // injection
            matrix(layout, DType::I32, 3 * tokens, 1);                   // MRoPE positions
            matrix(layout, DType::I32, 1, 1);                            // valid columns
            {
                auto nested = layout.scope();
                scratch(layout, ops::hyperconnection_mix_workspace_capacity_bytes(tokens, true));
            }
            {
                auto nested = layout.scope();
                scratch(layout,
                        ops::flash_next_qsa_workspace_capacity_bytes(tokens, plan.capacity));
            }
            {
                auto nested = layout.scope();
                scratch(layout, ops::flash_next_moe_workspace_capacity_bytes(tokens));
            }
            return;
        }
        ...
        if constexpr (Variant::flash_next) {
            scratch(layout, ops::flash_next_qsa_workspace_capacity_bytes(tokens, plan.capacity));
        } else {
            scratch(layout,
                    ops::causal_softmax_attention_workspace_capacity_bytes(
                        {TextConfig::head_dim, TextConfig::query_heads, TextConfig::kv_heads},
    ...
    const auto mtp_prefill_chunk = [&](WorkspaceLayoutBuilder& layout, std::int32_t first,
        ...
        if constexpr (Variant::flash_next) {
            ...
            (void)preembedded;
            matrix(layout, DType::BF16, TextConfig::hidden, last);
            matrix(layout, DType::BF16, 4 * TextConfig::hidden, last);
            mtp_full_core(layout, last, text_envelope);
            proposal_scratch(layout, 1);
            return;
        }
        ...
        matrix(layout, DType::BF16, TextConfig::query_size, 1);
        if constexpr (Variant::flash_next) {
            scratch(layout, ops::flash_next_qsa_workspace_capacity_bytes(1, plan.capacity));
        } else {
            scratch(layout,
                    ops::causal_softmax_attention_workspace_capacity_bytes(
                        {TextConfig::head_dim, TextConfig::query_heads, TextConfig::kv_heads},
    ...
    target_body(text_prefill, 1, chunk, qwen3_8_flash_next::TextPhase::Prefill,
    ...
    for (std::int32_t batch = 1; batch <= static_cast<std::int32_t>(plan.max_concurrency);
        ...
        target_body(ordinary, batch, batch, qwen3_8_flash_next::TextPhase::Verify,
    ...
    if (plan.features.mtp()) {
        ...
        target_body(mtp_prefill, 1, chunk, qwen3_8_flash_next::TextPhase::Prefill,
                    GdnWorkspacePath::Prefill, 1, 1, chunk, text_envelope);
        ...
        for (std::int32_t batch = 1; batch <= static_cast<std::int32_t>(plan.max_concurrency);
            ...
            target_body(target, aggregate, aggregate, qwen3_8_flash_next::TextPhase::Verify,
...
std::unique_ptr<qwen3_8_flash_next::detail::SequencePlannerImpl<Variant>>
make_sequence_planner_impl(DeviceContext& device, const EngineOptions& options,
    ...
    auto planner     = std::make_unique<qwen3_8_flash_next::detail::SequencePlannerImpl<Variant>>();
...
std::unique_ptr<SequencePlanImpl> finalize_sequence_plan_impl(
