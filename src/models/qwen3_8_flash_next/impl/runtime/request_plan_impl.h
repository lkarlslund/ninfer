...
runtime::PrefillWork rebuild_work_at_frontier(const PreparedPromptData& prompt,
    ...
    for (const qwen3_8_flash_next::VisionItem& item : prompt.vision_items) {
        bool fully_consumed = !item.token_spans.empty();
        for (const qwen3_8_flash_next::TokenSpan& span : item.token_spans) {
...
RequestBasePlan ProgramImplCore::plan_request(const PreparedPromptData& prompt,
    ...
    if (prompt.has_media()) {
        ...
        auto vision = std::make_shared<qwen3_8_flash_next::VisionControlPlan>(
            qwen3_8_flash_next::plan_vision_control(prompt));
        std::uint32_t previous_end = 0;
        for (std::size_t index = 0; index < vision->items.size(); ++index) {
            const qwen3_8_flash_next::VisionItemControlPlan& item = vision->items[index];
    ...
    if (options.allow_prefix_reuse && prompt.identity.reusable && context_cache.enabled) {
        ...
        for (const qwen3_8_flash_next::PreparedCacheOpportunity& opportunity :
        ...
        for (CaptureGroup& group : base->capture_groups) {
            ...
            identity->shortlist_key = qwen3_8_flash_next::PrefixShortlistKey{
...
std::optional<AdmissionCandidate> ProgramImplCore::inspect_lane(
    ...
    if (shared_source != nullptr) {
        ...
        if (shared_identity == nullptr ||
            !qwen3_8_flash_next::detail::prefix_matches(prompt, shared_source->identity->ledger(),
        ...
        if (selected.kind == runtime::CheckpointKind::SessionEndpoint) {
            ...
            if (!qwen3_8_flash_next::detail::prefix_matches(
    ...
    if (base.vision_control_plan) {
        ...
        for (std::size_t index = 0; index < base.vision_control_plan->items.size(); ++index) {
            const qwen3_8_flash_next::VisionItemControlPlan& item =
                base.vision_control_plan->items[index];
