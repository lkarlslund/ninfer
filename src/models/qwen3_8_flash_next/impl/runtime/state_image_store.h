...
public:
    ...
    void reset_active(StateImageHandle handle, cudaStream_t stream = nullptr) {
        Object& object = require(handle);
        if (object.role != StateImageRole::ActiveMutable || !object.device_slot ||
            object.source_pins != 0 || object.destination_pinned || has_pending_replica(object)) {
            throw std::logic_error("StateImage active image is not resettable");
        }
        device_->zero_slot(*object.device_slot, stream);
        object.content_epoch = next_epoch();
    }
    ...
    reserve_device_to_host(StateImageHandle source) {
        ...
        std::optional<qwen3_8_flash_next::HostStateSlotHandle> target = host_->allocate();
...
private:
    struct Object {
        ...
        std::optional<qwen3_8_flash_next::HostStateSlotHandle> host_slot;
        std::optional<std::int32_t> pending_device_slot;
        std::optional<qwen3_8_flash_next::HostStateSlotHandle> pending_host_slot;
    ...
    qwen3_8_flash_next::StateImageDevicePool* device_ = nullptr;
    qwen3_8_flash_next::HostStatePool* host_          = nullptr;
