...
SequencePlan<Variant> SequencePlanner<Variant>::finalize(std::uint32_t main_page_groups) && {
    ...
    return SequencePlan<Variant>(
        detail::NINFER_QWEN38_FLASH_NEXT_RUNTIME_NS::finalize_sequence_plan_impl(std::move(impl_),
                                                                                 main_page_groups));
}
...
template <>
PressureConstructionCursor PressurePlanningSession<Variant>::begin_construction(PressureTargetHandle target,
                                                                       bool restore) {
...
template <>
void PressurePlanningSession<Variant>::choose_construction(PressureConstructionCursor& cursor,
...
PressurePlanningSession<Variant>::prepare_expansion(PressureTargetHandle parent, std::uint32_t maximum_owners) {
    if (impl_ == nullptr) { throw std::logic_error("pressure planning session is empty"); }
...
SequencePlanner<Variant> make_sequence_planner<Variant>(DeviceContext& device,
    ...
    return SequencePlanner<Variant>(
        detail::NINFER_QWEN38_FLASH_NEXT_RUNTIME_NS::make_sequence_planner_impl(device, options,
                                                                                weights_profile));
}
