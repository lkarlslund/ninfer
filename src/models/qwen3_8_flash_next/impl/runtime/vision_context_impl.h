...
std::size_t VisionContext::workspace_bytes(const qwen3_8_flash_next::VisionItemControl& item) {
...
void VisionContext::encode(const VisionItemView& item, Tensor& output, DeviceSpan backing,
    ...
    const qwen3_8_flash_next::VisionItemControl& control = *item.control;
...
VisionPrefillSession::VisionPrefillSession(DeviceContext& device, const LoadedModelData& model,
                                           ...
                                           qwen3_8_flash_next::PreparedPromptData& prompt,
    ...
    for (const VisionUseSpan& use : plan_.uses) {
        ...
        const qwen3_8_flash_next::VisionItemControl& control =
            plan_.control->items[use.control_index];
        const qwen3_8_flash_next::VisionItem& source =
            prompt_.vision_items[use.prepared_item_index];
    ...
    if (plan_.max_merged_count != 0 &&
        std::none_of(plan_.control->items.begin(), plan_.control->items.end(),
                     [&](const qwen3_8_flash_next::VisionItemControl& item) {
...
VisionChunk VisionPrefillSession::prepare_chunk(std::uint32_t begin, std::uint32_t nominal_length) {
    ...
    const qwen3_8_flash_next::VisionItemControl& control =
        plan_.control->items[active->control_index];
