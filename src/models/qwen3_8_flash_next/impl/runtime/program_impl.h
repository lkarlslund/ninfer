...
#include "models/qwen3_8_flash_next/impl/runtime/program.h"
#include "models/qwen3_8_flash_next/impl/runtime/rebuild_work.h"
...
std::optional<qwen3_8_flash_next::TargetKVRequirement>
retained_requirement_after_drops(const qwen3_8_flash_next::ContinuationSummary& summary,
                                 std::span<const runtime::CheckpointRef> dropped) noexcept {
    if (dropped.empty()) { return std::nullopt; }
    qwen3_8_flash_next::TargetKVRequirement requirement;
    std::size_t found  = 0;
    bool surviving     = false;
    const auto include = [&](const qwen3_8_flash_next::CheckpointSummary& checkpoint) {
    ...
    for (const qwen3_8_flash_next::CheckpointSummary& anchor : summary.long_anchors) {
        include(anchor);
    }
...
std::optional<qwen3_8_flash_next::TargetKVRequirement>
retained_requirement_after_drop(const qwen3_8_flash_next::ContinuationSummary& summary,
...
void append_pressure_transfer(qwen3_8_flash_next::detail::PressureDecision& option,
...
bool pressure_state_drops_host(qwen3_8_flash_next::detail::PressureStateDecision change) noexcept {
    return change == qwen3_8_flash_next::detail::PressureStateDecision::DropEndpointHostDuplicate ||
           change == qwen3_8_flash_next::detail::PressureStateDecision::DropRewriteHostDuplicate ||
           change == qwen3_8_flash_next::detail::PressureStateDecision::DropSharedHostDuplicate;
}

bool pressure_state_demotes(qwen3_8_flash_next::detail::PressureStateDecision change) noexcept {
    return change == qwen3_8_flash_next::detail::PressureStateDecision::DemoteEndpointToHost ||
           change == qwen3_8_flash_next::detail::PressureStateDecision::DemoteRewriteToHost ||
           change == qwen3_8_flash_next::detail::PressureStateDecision::DemoteSharedToHost;
}

std::optional<StateImageHandle>
pressure_state_source(qwen3_8_flash_next::detail::PressureStateDecision change,
                      const SequenceState* sequence, const SharedPrefixState* shared) {
    switch (change) {
    case qwen3_8_flash_next::detail::PressureStateDecision::None:
        return std::nullopt;
    case qwen3_8_flash_next::detail::PressureStateDecision::DropEndpointDeviceDuplicate:
    case qwen3_8_flash_next::detail::PressureStateDecision::DemoteEndpointToHost:
    case qwen3_8_flash_next::detail::PressureStateDecision::DropEndpointHostDuplicate:
        if (sequence == nullptr) {
            throw std::logic_error("private pressure State action targets a shared owner");
        }
        return sequence->state.read;
    case qwen3_8_flash_next::detail::PressureStateDecision::DropRewriteDeviceDuplicate:
    case qwen3_8_flash_next::detail::PressureStateDecision::DemoteRewriteToHost:
    case qwen3_8_flash_next::detail::PressureStateDecision::DropRewriteHostDuplicate:
    ...
    case qwen3_8_flash_next::detail::PressureStateDecision::DropSharedDeviceDuplicate:
    case qwen3_8_flash_next::detail::PressureStateDecision::DemoteSharedToHost:
    case qwen3_8_flash_next::detail::PressureStateDecision::DropSharedHostDuplicate:
        if (shared == nullptr) {
...
qwen3_8_flash_next::detail::PressureDecision
combine_checkpoint_and_replica_target(qwen3_8_flash_next::detail::PressureDecision checkpoint,
                                      qwen3_8_flash_next::detail::PressureDecision replica) {
...
std::uint64_t
explicit_pressure_identity(const qwen3_8_flash_next::detail::PressureDecision& decision) noexcept {
    std::uint64_t identity = decision.shared_owner ? 0x5348415245445052ULL : 0x5052495641544550ULL;
    const auto mix         = [&](std::uint64_t value) {
        identity ^= value;
        identity *= 1099511628211ULL;
    };
    for (const qwen3_8_flash_next::detail::PressureStateDecision change : decision.state_changes) {
        mix(static_cast<std::uint8_t>(change));
    }
    const auto mix_kv = [&](std::span<const qwen3_8_flash_next::detail::PressureKVDecision> changes,
                            std::uint64_t tag) {
        for (const qwen3_8_flash_next::detail::PressureKVDecision& change : changes) {
            mix(tag);
...
qwen3_8_flash_next::detail::PressureDecision
explicit_pressure_target(const qwen3_8_flash_next::detail::PressureDecision& complete) {
    qwen3_8_flash_next::detail::PressureDecision explicit_target;
...
struct KVPressureSelection {
    std::vector<qwen3_8_flash_next::detail::PressureKVDecision> actions;
...
KVPressureSelection select_kv_pressure_actions(
    const KVAddressSpaceStore& addresses, LogicalKVPageStore& pages,
    HostKVExtentStore* host_extents, bool host_allocation_available, KVAddressSpaceHandle address,
    std::optional<std::uint32_t> mapped_limit, std::uint32_t requested_device_pages,
    std::size_t requested_host_bytes, runtime::ContextResourceClass resource,
    std::span<const qwen3_8_flash_next::detail::PressureKVDecision> existing_actions,
    ...
    for (const qwen3_8_flash_next::detail::PressureKVDecision& action : existing_actions) {
        if (action.kind == qwen3_8_flash_next::detail::PressureKVDecisionKind::None ||
    ...
    const auto mark_action = [&](qwen3_8_flash_next::detail::PressureKVDecision action) {
        for (std::uint32_t offset = 0; offset < action.page_count; ++offset) {
            const std::uint32_t page = action.begin_page + offset;
        ...
        while (end != 0 && !found) {
            const auto eligible = [&](std::uint32_t page) {
                ...
                return selected[page] == 0 && pages.device_resident(logical) &&
                       ...
                       !protected_page(
                           page, logical,
                           qwen3_8_flash_next::detail::PressureKVDecisionKind::DropHostDuplicate);
            };
            ...
            mark_action({
                ...
                .page_count = count,
                .kind       = qwen3_8_flash_next::detail::PressureKVDecisionKind::DropHostDuplicate,
            });
    ...
    const auto select_device_runs  = [&](bool require_host) {
        ...
        while (device_remaining != 0 && end != 0) {
            const auto eligible = [&](std::uint32_t page) {
                ...
                const qwen3_8_flash_next::detail::PressureKVDecisionKind action =
                    require_host
                        ? qwen3_8_flash_next::detail::PressureKVDecisionKind::DropDeviceDuplicate
                        : qwen3_8_flash_next::detail::PressureKVDecisionKind::DemoteToHost;
            ...
            qwen3_8_flash_next::detail::PressureKVDecision action{
...
DecodeGraphExecutable& install_graph_profile(DecodeGraphFamily& family, DecodeGraphProfile& profile,
    ...
    if (topology.installed_profile != profile_index) {
        try {
            ...
            throw std::runtime_error(std::string(label) + " CUDA Graph profile " +
                                     std::to_string(profile_index) + " (topology class " +
    ...
    const auto install_and_upload = [&](DecodeGraphTopology& topology, std::size_t profile_index) {
        ...
        if (topology.installed_profile != profile_index) {
            try {
                ...
                throw std::runtime_error(std::string(label) + " CUDA Graph profile " +
                                         std::to_string(profile_index) + " (topology class " +
                                         std::to_string(profile.topology_class) +
                                         "): " + error.what());
            }
            topology.installed_profile = profile_index;
        }
...
ProgramImplCore::ProgramImplCore(const LoadedModelData& model_in, const SequencePlanImpl& plan,
    ...
    : model(model_in), device(device_in), capacity(plan.capacity), kv_capacity(plan.kv_capacity),
      ...
      score_logprobs_host(plan.causal_scoring ? std::make_optional<PinnedHostBuffer>(
                                              ...
                                              : std::nullopt),
      ordinary_host(plan.speculative_backend == SpeculativeBackend::None
                        ? std::make_optional<PinnedHostBuffer>(
                              sizeof(qwen3_8_flash_next::OrdinaryDecodeIngress) +
      ...
      dflash_host(plan.speculative_backend == SpeculativeBackend::DFlash
                      ? std::make_optional<PinnedHostBuffer>(
                            ...
                            sizeof(qwen3_8_flash_next::DFlashDecodeEgress))
                      : std::nullopt),
    ...
    decoder = std::make_unique<qwen3_8_flash_next::DecoderState>(backing, plan.persistent.decoder);
    ...
    state_images = std::make_unique<qwen3_8_flash_next::StateImageDevicePool>(
        backing, plan.persistent.state_images);
    if (plan.context_cache.host_state_slots != 0) {
        ...
        host_state_images = std::make_unique<qwen3_8_flash_next::HostStatePool>(
    ...
    if (qwen3_8_flash_next::PagedKVCache* backend = backend_kv_cache()) {
    ...
    if (plan.context_cache.host_kv_capacity_bytes != 0) {
        ...
        if (const qwen3_8_flash_next::PagedKVCache* backend = backend_kv_cache()) {
    ...
    if (plan.persistent.mtp_prefill_hidden) {
        mtp_prefill_hidden.emplace(plan.persistent.mtp_prefill_hidden->bind(backing));
    }
    ...
    if (ordinary_host) {
        ...
        ordinary_host_egress = reinterpret_cast<qwen3_8_flash_next::OrdinaryDecodeEgress*>(
    ...
    if (dflash_host) {
        dflash_host_ingress =
            static_cast<qwen3_8_flash_next::DFlashDecodeIngress*>(dflash_host->data());
        dflash_host_egress = reinterpret_cast<qwen3_8_flash_next::DFlashDecodeEgress*>(
        ...
        while (cursor < predictor_count) {
            ...
            schedule::PrefillContext schedule_state{
                {device, model, work, state_images->linear(), state_images->ple_store(),
                 flash_ple_records ? &*flash_ple_records : nullptr, nullptr, io, prefill_hidden,
                 prefill_chunk, proposal_head, mtp_prefill_hidden ? &*mtp_prefill_hidden : nullptr,
...
std::optional<qwen3_8_flash_next::detail::PressureDecision>
ProgramImplCore::inspect_shared_pressure_option(
    const SharedPrefixState& shared, detail::PhysicalResources deficit,
    const MaterializationSourceProtection* protection,
    const qwen3_8_flash_next::detail::PressureDecision* current) const {
    ...
    qwen3_8_flash_next::detail::PressureDecision option;
    ...
    for (const qwen3_8_flash_next::detail::PressureStateDecision change : option.state_changes) {
        mix(static_cast<std::uint8_t>(change));
    }
    const auto mix_existing_kv =
        [&](std::span<const qwen3_8_flash_next::detail::PressureKVDecision> actions,
            std::uint64_t tag) {
            for (const qwen3_8_flash_next::detail::PressureKVDecision& action : actions) {
                mix(tag);
    ...
    qwen3_8_flash_next::detail::PressureStateDecision state_change =
        qwen3_8_flash_next::detail::PressureStateDecision::None;
    if (option.state_changes.empty() &&
        ...
        if (deficit.host.state_slots != 0 && residency == StateReplicaResidency::Both) {
            state_change =
                qwen3_8_flash_next::detail::PressureStateDecision::DropSharedHostDuplicate;
            ...
            state_change =
                qwen3_8_flash_next::detail::PressureStateDecision::DropSharedDeviceDuplicate;
        } else if (!protected_state && state_store->checkpoint_references(shared.state) == 1 &&
                   deficit.device.state_slots != 0 &&
                   residency == StateReplicaResidency::DeviceOnly && host_state_images != nullptr) {
            state_change = qwen3_8_flash_next::detail::PressureStateDecision::DemoteSharedToHost;
        ...
        if (state_change != qwen3_8_flash_next::detail::PressureStateDecision::None) {
            if (state_change !=
                qwen3_8_flash_next::detail::PressureStateDecision::DropSharedHostDuplicate) {
    ...
    const auto add_kv = [&](const KVAddressSpaceStore& addresses, LogicalKVPageStore& pages,
                            ...
                            std::vector<qwen3_8_flash_next::detail::PressureKVDecision>& changes,
        ...
        KVPressureSelection selected = select_kv_pressure_actions(
            ...
            [&](std::uint32_t page, LogicalKVPageHandle logical,
                qwen3_8_flash_next::detail::PressureKVDecisionKind action) {
                return action !=
                           qwen3_8_flash_next::detail::PressureKVDecisionKind::DropHostDuplicate &&
        ...
        for (const qwen3_8_flash_next::detail::PressureKVDecision& action : selected.actions) {
            mix(tag);
            mix(action.begin_page);
            mix(action.page_count);
            mix(static_cast<std::uint8_t>(action.kind));
...
std::vector<qwen3_8_flash_next::detail::PressureDecision>
ProgramImplCore::inspect_shared_pressure_options(
    const SharedPrefixState& shared, detail::PhysicalResources deficit,
    const MaterializationSourceProtection* protection,
    const qwen3_8_flash_next::detail::PressureDecision* current) const {
    ...
    std::vector<qwen3_8_flash_next::detail::PressureDecision> options;
    options.reserve(endpoints.size());
    for (const detail::PhysicalResources requested : endpoints) {
        std::optional<qwen3_8_flash_next::detail::PressureDecision> option =
...
std::optional<qwen3_8_flash_next::detail::PressureDecision>
ProgramImplCore::inspect_pressure_option(
    const SequenceState& sequence, detail::PhysicalResources deficit,
    const MaterializationSourceProtection* protection,
    const qwen3_8_flash_next::TargetKVRequirement* retained_requirement,
    ...
    const auto mix_kv = [&](std::span<const qwen3_8_flash_next::detail::PressureKVDecision> actions,
                            std::uint64_t tag) {
        for (const qwen3_8_flash_next::detail::PressureKVDecision& action : actions) {
            mix(tag);
            mix(action.begin_page);
    ...
    const auto add_state = [&](StateImageHandle state, bool rewrite) {
        ...
        const qwen3_8_flash_next::detail::PressureStateDecision endpoint_drop =
            rewrite
                ? qwen3_8_flash_next::detail::PressureStateDecision::DropRewriteDeviceDuplicate
                : qwen3_8_flash_next::detail::PressureStateDecision::DropEndpointDeviceDuplicate;
        const qwen3_8_flash_next::detail::PressureStateDecision endpoint_demote =
            rewrite ? qwen3_8_flash_next::detail::PressureStateDecision::DemoteRewriteToHost
                    : qwen3_8_flash_next::detail::PressureStateDecision::DemoteEndpointToHost;
        const qwen3_8_flash_next::detail::PressureStateDecision endpoint_host_drop =
            rewrite ? qwen3_8_flash_next::detail::PressureStateDecision::DropRewriteHostDuplicate
                    : qwen3_8_flash_next::detail::PressureStateDecision::DropEndpointHostDuplicate;
        ...
        qwen3_8_flash_next::detail::PressureStateDecision change =
            qwen3_8_flash_next::detail::PressureStateDecision::None;
...
std::vector<qwen3_8_flash_next::detail::PressureDecision>
ProgramImplCore::inspect_pressure_successors(
    const SequenceState& sequence, detail::PhysicalResources residual,
    const MaterializationSourceProtection* protection,
    const qwen3_8_flash_next::detail::PressureDecision* current) const {
    std::vector<qwen3_8_flash_next::detail::PressureDecision> successors;
    if (!sequence.kv || (current != nullptr && current->evicts_continuation)) { return successors; }
    const auto append_unique = [&](qwen3_8_flash_next::detail::PressureDecision option) {
    ...
    const qwen3_8_flash_next::ContinuationSummary summary = continuation_summary(sequence);
    ...
    const auto state_change_conflicts = [](runtime::CheckpointRef checkpoint,
                                           const qwen3_8_flash_next::detail::PressureDecision&
                                               explicit_target) {
        const auto has = [&](qwen3_8_flash_next::detail::PressureStateDecision change) {
        ...
        if (checkpoint.kind == runtime::CheckpointKind::SessionEndpoint) {
            return has(qwen3_8_flash_next::detail::PressureStateDecision::
                           DropEndpointDeviceDuplicate) ||
                   has(qwen3_8_flash_next::detail::PressureStateDecision::DemoteEndpointToHost) ||
                   has(qwen3_8_flash_next::detail::PressureStateDecision::
        ...
        if (checkpoint.kind == runtime::CheckpointKind::TurnClosure ||
            ...
            return has(qwen3_8_flash_next::detail::PressureStateDecision::
                   ...
                   has(qwen3_8_flash_next::detail::PressureStateDecision::DemoteRewriteToHost) ||
                   has(qwen3_8_flash_next::detail::PressureStateDecision::DropRewriteHostDuplicate);
        }
        return false;
    };
    const auto append_checkpoint_successor = [&](runtime::CheckpointRef checkpoint) {
        ...
        std::optional<qwen3_8_flash_next::detail::PressureDecision> drop =
        ...
        qwen3_8_flash_next::detail::PressureDecision explicit_target =
            explicit_pressure_target(*current);
        if (state_change_conflicts(checkpoint, explicit_target)) { return; }
        const std::optional<qwen3_8_flash_next::TargetKVRequirement> retained =
            retained_requirement_after_drops(summary, drop->dropped_checkpoints);
        if (!retained) { return; }
        const auto within =
            [](std::span<const qwen3_8_flash_next::detail::PressureKVDecision> changes,
               std::uint32_t pages) {
                return std::all_of(changes.begin(), changes.end(), [&](const auto& action) {
                    return action.kind !=
                               qwen3_8_flash_next::detail::PressureKVDecisionKind::None &&
    ...
    const std::optional<qwen3_8_flash_next::TargetKVRequirement> retained =
        dropped.empty() ? std::optional<qwen3_8_flash_next::TargetKVRequirement>()
    ...
    for (const detail::PhysicalResources requested : endpoints) {
        std::optional<qwen3_8_flash_next::detail::PressureDecision> replica =
            inspect_pressure_option(sequence, requested, protection,
                                    retained ? &*retained : nullptr, dropped, {}, current);
        if (!replica) { continue; }
        if (dropped.empty()) {
...
std::vector<qwen3_8_flash_next::detail::PressureDecision>
ProgramImplCore::inspect_shared_pressure_successors(
...
std::vector<runtime::ContextTransferRequirement> ProgramImplCore::checkpoint_restore_requirements(
    const SequenceKVBundle& kv, const qwen3_8_flash_next::TargetKVRequirement& requirement,
...
bool ProgramImplCore::pressure_checkpoint_recovery_impacts(
    ...
    std::span<const qwen3_8_flash_next::detail::PressureDecision* const> private_decisions,
    std::span<const runtime::PlanningOwnerId> private_owner_ids,
    std::span<const SharedPrefixHandle* const> shared_owners,
    std::span<const qwen3_8_flash_next::detail::PressureDecision* const> shared_decisions,
    std::span<const runtime::PlanningOwnerId> shared_owner_ids,
    std::vector<qwen3_8_flash_next::detail::PressureCheckpointRecoveryProjection>& output,
    ...
    const auto apply_kv_action =
        [&](const KVAddressSpaceStore* addresses, const LogicalKVPageStore* pages,
            std::optional<KVAddressSpaceHandle> address,
            const qwen3_8_flash_next::detail::PressureKVDecision& action) -> bool {
        if (action.kind == qwen3_8_flash_next::detail::PressureKVDecisionKind::None) {
        ...
        const bool drops_host =
            action.kind == qwen3_8_flash_next::detail::PressureKVDecisionKind::DropHostDuplicate;
    ...
    const auto apply_decision =
        [&](const SequenceState* sequence, const SharedPrefixState* shared,
            const qwen3_8_flash_next::detail::PressureDecision* decision) -> bool {
        if (decision == nullptr || decision->evicts_continuation) { return true; }
        for (const qwen3_8_flash_next::detail::PressureStateDecision change :
             decision->state_changes) {
            std::optional<StateImageHandle> state;
            switch (change) {
            case qwen3_8_flash_next::detail::PressureStateDecision::None:
                return false;
            ...
            const bool drops_host =
                change ==
                    qwen3_8_flash_next::detail::PressureStateDecision::DropEndpointHostDuplicate ||
                change ==
                    qwen3_8_flash_next::detail::PressureStateDecision::DropRewriteHostDuplicate ||
                change ==
                    qwen3_8_flash_next::detail::PressureStateDecision::DropSharedHostDuplicate;
        ...
        for (const qwen3_8_flash_next::detail::PressureKVDecision& action :
    ...
    for (const OwnerProjection& owner : projected_owners) {
        ...
        if (owner.sequence != nullptr) {
            qwen3_8_flash_next::ContinuationSummary& summary = scratch.continuation_summary;
            ...
            const qwen3_8_flash_next::CheckpointSummary checkpoint =
        ...
        const auto append_target_recovery_work = [&](std::size_t selected) {
            ...
            output.push_back(qwen3_8_flash_next::detail::PressureCheckpointRecoveryProjection{
                .owner              = owner.owner,
...
std::optional<qwen3_8_flash_next::detail::PressureDecision>
ProgramImplCore::inspect_checkpoint_drop_option(
    ...
    const std::optional<qwen3_8_flash_next::TargetKVRequirement> remaining =
...
bool ProgramImplCore::pressure_decision_valid(
    const SequenceState& sequence, const qwen3_8_flash_next::detail::PressureDecision& decision,
    ...
    std::optional<qwen3_8_flash_next::TargetKVRequirement> retained;
    if (!decision.dropped_checkpoints.empty()) {
        const std::optional<qwen3_8_flash_next::detail::PressureDecision> canonical =
    ...
    } else if (decision.checkpoint_drop_effect != qwen3_8_flash_next::detail::PhysicalDelta{}) {
        return false;
    }
    ...
    for (const qwen3_8_flash_next::detail::PressureStateDecision action : decision.state_changes) {
        const bool endpoint =
            action ==
                qwen3_8_flash_next::detail::PressureStateDecision::DropEndpointDeviceDuplicate ||
            action == qwen3_8_flash_next::detail::PressureStateDecision::DemoteEndpointToHost ||
            action == qwen3_8_flash_next::detail::PressureStateDecision::DropEndpointHostDuplicate;
        const bool rewrite =
            action ==
                qwen3_8_flash_next::detail::PressureStateDecision::DropRewriteDeviceDuplicate ||
            action == qwen3_8_flash_next::detail::PressureStateDecision::DemoteRewriteToHost ||
            action == qwen3_8_flash_next::detail::PressureStateDecision::DropRewriteHostDuplicate;
        if ((!endpoint && !rewrite) ||
    ...
    const auto valid_kv = [&](const KVAddressSpaceStore* addresses, LogicalKVPageStore* pages,
                              std::optional<KVAddressSpaceHandle> address,
                              std::span<const qwen3_8_flash_next::detail::PressureKVDecision>
                                  changes,
        ...
        for (const qwen3_8_flash_next::detail::PressureKVDecision& action : changes) {
            if (action.kind == qwen3_8_flash_next::detail::PressureKVDecisionKind::None ||
            ...
            for (std::uint32_t offset = 0; offset < action.page_count; ++offset) {
                ...
                if (action.kind ==
                    ...
                    return false;
                } else if (action.kind == qwen3_8_flash_next::detail::PressureKVDecisionKind::
                                              DropDeviceDuplicate) {
...
bool ProgramImplCore::shared_pressure_decision_valid(
    const SharedPrefixState& shared, const qwen3_8_flash_next::detail::PressureDecision& decision,
    const MaterializationSourceProtection* protection) const {
    if (!shared.kv || shared.active_references != 0 || decision.evicts_continuation ||
        ...
        decision.checkpoint_drop_effect != qwen3_8_flash_next::detail::PhysicalDelta{} ||
        decision.state_changes.size() > 1) {
        return false;
    }
    if (!decision.state_changes.empty()) {
        const qwen3_8_flash_next::detail::PressureStateDecision action =
            decision.state_changes.front();
        const std::optional<StateImageHandle> state =
...
void ProgramImplCore::publish_checkpoint_drop(SequenceState& sequence,
    ...
    const qwen3_8_flash_next::ContinuationSummary before = continuation_summary(sequence);
    const std::optional<qwen3_8_flash_next::TargetKVRequirement> retained =
    ...
    if (checkpoint.kind == runtime::CheckpointKind::SessionEndpoint) {
        ...
        sequence.rewrite_checkpoint_mtp_hidden = {};
...
std::optional<detail::PressureTargetProjection> ProgramImplCore::evaluate_pressure_target(
    ...
    std::span<const qwen3_8_flash_next::detail::PressureDecision> pressure_options,
    std::span<const SharedPrefixHandle* const> shared_pressure_owners,
    std::span<const qwen3_8_flash_next::detail::PressureDecision> shared_pressure_options,
    ...
    const auto append_pressure_targets = [&](const qwen3_8_flash_next::detail::PressureDecision&
                                                 option,
                                             const SequenceState* sequence,
                                             const SharedPrefixState* shared) {
        for (const qwen3_8_flash_next::detail::PressureStateDecision change :
             option.state_changes) {
            std::optional<StateImageHandle> state;
            switch (change) {
            case qwen3_8_flash_next::detail::PressureStateDecision::None:
                return false;
        ...
        const auto append_pages = [&](const KVAddressSpaceStore* addresses,
                                      ...
                                      std::optional<KVAddressSpaceHandle> address,
                                      const qwen3_8_flash_next::detail::PressureKVDecision&
                                          action) {
            if (action.kind == qwen3_8_flash_next::detail::PressureKVDecisionKind::None) {
                return action.page_count == 0;
            }
    ...
    for (std::size_t position = 0; position < pressure_owners.size(); ++position) {
        ...
        const qwen3_8_flash_next::detail::PressureDecision& option = pressure_options[position];
    ...
    for (std::size_t position = 0; position < shared_pressure_owners.size(); ++position) {
        ...
        const qwen3_8_flash_next::detail::PressureDecision& option =
            shared_pressure_options[position];
        if (owner == nullptr || ContractAccess::owner(*owner) != this ||
...
std::optional<AdmissionCandidate> ProgramImplCore::seal_materialization(
    ...
    std::span<const qwen3_8_flash_next::detail::PressureDecision* const> pressure_options,
    std::span<const SharedPrefixHandle* const> shared_pressure_owners,
    std::span<const runtime::PlanningOwnerId> shared_pressure_owner_ids,
    std::span<const qwen3_8_flash_next::detail::PressureDecision* const> shared_pressure_options) {
...
bool ProgramImplCore::compose_pressure_candidate(
    ...
    const auto demotion_count = [](const qwen3_8_flash_next::detail::PressureDecision& option) {
    ...
    for (const qwen3_8_flash_next::detail::PressureDecision* option : pressure_options) {
        if (option == nullptr) {
    ...
    for (const qwen3_8_flash_next::detail::PressureDecision* option : shared_pressure_options) {
        if (option == nullptr) {
    ...
    const auto append_host_releases =
        [&](KVAddressSpaceStore& addresses, LogicalKVPageStore& pages, KVAddressSpaceHandle address,
            const qwen3_8_flash_next::detail::PressureKVDecision& action) {
            if (action.kind !=
                qwen3_8_flash_next::detail::PressureKVDecisionKind::DropHostDuplicate) {
                return;
            }
    ...
    for (std::size_t position = 0; position < pressure_options.size(); ++position) {
        ...
        const qwen3_8_flash_next::detail::PressureDecision& proposed = *pressure_options[position];
        if (owner == nullptr || ContractAccess::owner(*owner) != this ||
        ...
        qwen3_8_flash_next::detail::PressureDecision expected;
    ...
    for (std::size_t position = 0; position < shared_pressure_options.size(); ++position) {
        ...
        const qwen3_8_flash_next::detail::PressureDecision& proposed =
            *shared_pressure_options[position];
        if (owner == nullptr || ContractAccess::owner(*owner) != this ||
            planning_owner.value == std::numeric_limits<std::uint32_t>::max() ||
            std::find(details.pressure_owner_ids.begin(), details.pressure_owner_ids.end(),
...
ProgramImplCore::revalidate_materialization(const AdmissionCandidate& plan,
    ...
    if (source_state != nullptr &&
        !qwen3_8_flash_next::detail::prefix_matches(
            prompt, source_state->ledger, source_state->prefix_identity, details.reuse_base)) {
        return runtime::PreflightStatus::StalePolicyState;
    }
    if (shared_state != nullptr &&
        (!shared_state->identity || shared_state->identity->prefix_identity() == nullptr ||
         !qwen3_8_flash_next::detail::prefix_matches(prompt, shared_state->identity->ledger(),
...
ProgramImplCore::reserve_materialization(AdmissionCandidate&& plan, PreparedPromptData&& prompt,
    ...
    try {
        ...
        if (source_state != nullptr &&
            !qwen3_8_flash_next::detail::prefix_matches(prompt, source_state->ledger,
        ...
        for (const CaptureGroup& group : request_plan.capture_groups) {
            ...
            if (!group.identity ||
                ...
                !qwen3_8_flash_next::detail::prefix_matches(prompt, group.identity->ledger(),
        ...
        if (request_plan.vision) {
            ...
            auto control = std::make_shared<qwen3_8_flash_next::VisionControl>(
                qwen3_8_flash_next::build_vision_control(prompt, *vision.control_plan, first_item));
...
void ProgramImplCore::prepare_consumed_source(MaterializationTransaction& transaction) {
    ...
    if (details.reuse == ReusePath::PrivateEndpoint &&
        ...
        source.rewrite_checkpoint_mtp_hidden = {};
...
void ProgramImplCore::publish_pressure_host_releases(
    ...
    for (std::size_t index = 0; index < work.option.state_changes.size(); ++index) {
        const qwen3_8_flash_next::detail::PressureStateDecision action =
            work.option.state_changes[index];
    ...
    const auto release_kv = [&](KVAddressSpaceStore& addresses, LogicalKVPageStore& pages,
        ...
        if (action.kind != qwen3_8_flash_next::detail::PressureKVDecisionKind::DropHostDuplicate ||
    ...
    const auto prepare_kv = [&](KVAddressSpaceStore& addresses, LogicalKVPageStore& pages,
        ...
        for (std::uint32_t offset = 0; offset < action.page_count; ++offset) {
            ...
            const bool valid_residency =
                action.kind == qwen3_8_flash_next::detail::PressureKVDecisionKind::DemoteToHost
                    ? !host_resident
                    : host_resident;
...
bool ProgramImplCore::can_retain_rewrite_checkpoint(const PreparedPromptData& prompt,
    ...
    if (!sequence.rewrite_checkpoint.valid || !sequence.rewrite_state ||
        !state_store->valid(*sequence.rewrite_state) ||
        !qwen3_8_flash_next::detail::prefix_matches(prompt, sequence.ledger,
...
qwen3_8_flash_next::CheckpointSummary
ProgramImplCore::checkpoint_summary(const SequenceState& sequence,
    ...
    return qwen3_8_flash_next::CheckpointSummary{
        .ref   = checkpoint,
...
qwen3_8_flash_next::ContinuationSummary
ProgramImplCore::continuation_summary(const SequenceState& sequence) const {
    qwen3_8_flash_next::ContinuationSummary summary;
    summary.long_anchors.reserve(sequence.long_anchors.size());
    populate_continuation_summary(sequence, summary);
    return summary;
}

void ProgramImplCore::populate_continuation_summary(
    const SequenceState& sequence, qwen3_8_flash_next::ContinuationSummary& summary) const {
...
qwen3_8_flash_next::SharedPrefixSummary
ProgramImplCore::shared_prefix_summary(const SharedPrefixState& shared) const {
    ...
    return qwen3_8_flash_next::SharedPrefixSummary{
        .checkpoint =
            {
                .ref =
                    {
...
ProgramImplCore::inspect_capture(const CaptureOffer& offer, const SharedPrefixHandle* exact_shared,
    ...
    if (device_destination_available || host_state_images == nullptr) {
        assessment.state_placement = qwen3_8_flash_next::CaptureStatePlacement::DeviceFork;
        ...
        assessment.state_placement = qwen3_8_flash_next::CaptureStatePlacement::HostSnapshot;
    ...
    if (assessment.recycles_private_state) {
        if (assessment.state_placement != qwen3_8_flash_next::CaptureStatePlacement::DeviceFork) {
    ...
    if (assessment.state_placement == qwen3_8_flash_next::CaptureStatePlacement::HostSnapshot) {
    ...
    if (publish_shared) {
        ...
        assessment.projected_recovery_work.push_back(
            NINFER_QWEN38_FLASH_NEXT_RUNTIME_NS::recovery_alternative_work(
                {}, assessment.protected_rebuild_work));
        assessment.projected_recovery_work.push_back(
            NINFER_QWEN38_FLASH_NEXT_RUNTIME_NS::recovery_alternative_work(recovery));
    }
    return assessment;
}
...
ProgramImplCore::checkpoint_recovery_work(const ContinuationHandle& owner,
    ...
    struct RecoverySource {
        const qwen3_8_flash_next::CheckpointSummary* checkpoint = nullptr;
        StateImageHandle state;
    };
    ...
    alternatives.push_back(NINFER_QWEN38_FLASH_NEXT_RUNTIME_NS::recovery_alternative_work(
...
ProgramImplCore::checkpoint_recovery_work(const SharedPrefixHandle& owner,
    ...
    const qwen3_8_flash_next::CheckpointSummary summary = shared_prefix_summary(shared).checkpoint;
    ...
    alternatives.push_back(
        NINFER_QWEN38_FLASH_NEXT_RUNTIME_NS::recovery_alternative_work({}, summary.rebuild_work));
    alternatives.push_back(NINFER_QWEN38_FLASH_NEXT_RUNTIME_NS::recovery_alternative_work(
...
ProgramImplCore::make_capture_physical_candidate(const CaptureAssessment& assessment) const {
    ...
    details->identity_assessment.machine_work =
        NINFER_QWEN38_FLASH_NEXT_RUNTIME_NS::materialization_machine_work(*details, {}, {});
...
void ProgramImplCore::prepare_active_capture(ActiveCaptureTransaction& transaction) {
    ...
    if (transaction.state_placement == qwen3_8_flash_next::CaptureStatePlacement::HostSnapshot) {
    ...
    if (transaction.state_placement == qwen3_8_flash_next::CaptureStatePlacement::DeviceFork) {
...
void ProgramImplCore::abort_active_capture(ActiveCaptureTransaction& transaction) noexcept {
    if (transaction.lane < max_concurrency &&
        ...
        if (state_store->valid(transaction.source_state) &&
            ...
            try {
                if (transaction.state_placement ==
                    qwen3_8_flash_next::CaptureStatePlacement::HostSnapshot) {
...
ProgramImplCore::progress_active_capture_transaction(runtime::CancellationFlagView cancellation) {
    ...
    if (transaction.transfer_submitted) {
        ...
        const runtime::ContextTransferDirection state_direction =
            transaction.state_placement == qwen3_8_flash_next::CaptureStatePlacement::HostSnapshot
                ? runtime::ContextTransferDirection::DeviceToHost
                : runtime::ContextTransferDirection::DeviceToDevice;
    ...
    try {
        for (std::size_t row = 0; row < members.size(); ++row) {
            ...
            while (cursor < end) {
                ...
                Tensor tail_mtp_hidden;
                if (mtp_prefill_hidden) {
                    tail_mtp_hidden = mtp_prefill_hidden->slice(
                        1, static_cast<std::int32_t>(result.processed_tokens) - 1, 1);
                }
                copy_tail(sequence,
                          prefill_hidden.slice(
                              1, static_cast<std::int32_t>(result.processed_tokens) - 1, 1),
                          tail_mtp_hidden.data != nullptr ? &tail_mtp_hidden : nullptr);
            }
            timing.begin_wait();
            device.synchronize();
            timing.end_wait();
...
detail::PhysicalResources ProgramImplCore::admission_capacity() const noexcept {
    const qwen3_8_flash_next::PagedKVCache* backend = backend_kv_cache();
    return detail::PhysicalResources{
        .device =
            {
...
qwen3_8_flash_next::PhysicalUsageSnapshot ProgramImplCore::physical_usage() const noexcept {
    const detail::PhysicalResources usage = physical_occupancy();
    return qwen3_8_flash_next::PhysicalUsageSnapshot{
...
void ProgramImplCore::refresh_state_views(SequenceState& sequence) {
    ...
    if (state_store->valid(sequence.state.read) && state_store->valid(sequence.state.write) &&
        ...
        if (state_images->mtp_hidden_store() != nullptr) {
            sequence.tail_mtp_hidden =
                state_images->mtp_hidden_slot(state_store->physical_slot(committed));
        }
    }
    if (sequence.rewrite_state && state_store->valid(*sequence.rewrite_state) &&
        ...
        if (state_images->mtp_hidden_store() != nullptr) {
            sequence.rewrite_checkpoint_mtp_hidden =
                state_images->mtp_hidden_slot(state_store->physical_slot(*sequence.rewrite_state));
        }
    }
}
...
qwen3_8_flash_next::PagedKVCache* ProgramImplCore::backend_kv_cache() noexcept {
...
const qwen3_8_flash_next::PagedKVCache* ProgramImplCore::backend_kv_cache() const noexcept {
    if (speculative_backend == SpeculativeBackend::Mtp) { return decoder->mtp_cache(); }
    if (speculative_backend == SpeculativeBackend::DFlash && dflash) { return &dflash->full; }
    return nullptr;
}
...
qwen3_8_flash_next::PagedKVCacheView
ProgramImplCore::text_kv_view(const SequenceState& sequence) const {
...
qwen3_8_flash_next::PagedKVCacheView
ProgramImplCore::mtp_kv_view(const SequenceState& sequence) const {
...
void ProgramImplCore::ordered_reset(SequenceState& sequence) {
    ...
    state_store->reset_active(sequence.state.write, device.stream);
    refresh_state_views(sequence);
    work.reset();
...
void ProgramImplCore::prepare_graphs() {
    ...
    const auto reserve_capture_rows = [&](qwen3_8_flash_next::PagedKVCache& cache,
    ...
    const auto zero_capture_pages =
        [&](qwen3_8_flash_next::PagedKVCache& cache, const KVAddressSpaceStore& addresses,
    ...
    const auto execution_core = [&] {
        return schedule::ExecutionCore{device,
                                       ...
                                       prefill_chunk,
                                       proposal_head,
                                       mtp_prefill_hidden ? &*mtp_prefill_hidden : nullptr,
                                       state_images->mtp_hidden_store(),
                                       bf16_gemm.get()};
    };
    ...
    if (speculative_backend == SpeculativeBackend::None) {
        ...
        schedule::OrdinaryBatchContext ordinary_state{execution_core(),
                                                      ...
                                                      *ordinary_host_egress,
                                                      ...
                                                      flash_decode_ple ? &*flash_decode_ple
                                                                       : nullptr};
...
void ProgramImplCore::copy_tail(SequenceState& sequence, const Tensor& source,
    ...
    CUDA_CHECK(cudaMemcpyAsync(sequence.tail_hidden.data, source.data, sequence.tail_hidden.bytes(),
                               cudaMemcpyDeviceToDevice, device.stream));
    if (sequence.tail_mtp_hidden.data != nullptr) {
        if (mtp_source == nullptr || mtp_source->dtype != DType::BF16 ||
            mtp_source->ne[0] != 4 * TextConfig::hidden || mtp_source->ne[1] != 1) {
            throw std::logic_error("target tail MTP hidden has an invalid shape");
        }
        CUDA_CHECK(cudaMemcpyAsync(sequence.tail_mtp_hidden.data, mtp_source->data,
                                   sequence.tail_mtp_hidden.bytes(), cudaMemcpyDeviceToDevice,
                                   device.stream));
    }
    sequence.tail_hidden_valid = true;
}
...
ProgramImplCore::advance_prefill(SequenceState& sequence, RequestControl& request,
    ...
    try {
        ...
        Tensor rewrite_capture_mtp_hidden;
        Tensor* rewrite_capture_mtp_hidden_ptr = nullptr;
        if (staged.next_capture < staged.capture_groups.size()) {
            ...
            if (state_images->mtp_hidden_store() != nullptr) {
                rewrite_capture_mtp_hidden = state_images->mtp_hidden_slot(selectors.destination);
                rewrite_capture_mtp_hidden_ptr = &rewrite_capture_mtp_hidden;
            }
        }
        schedule::PrefillContext schedule_state{
            {device, model, work, state_images->linear(), state_images->ple_store(),
            ...
            rewrite_capture_mtp_hidden_ptr,
            selectors.source,
            selectors.destination,
        ...
        if (staged.mtp_bridge == MtpBridgeMode::BeforeSuffix) {
            ...
            const Tensor& previous_hidden = sequence.tail_mtp_hidden.data != nullptr
                                                ? sequence.tail_mtp_hidden
                                                : sequence.tail_hidden;
        ...
        if (staged.cursor < staged.prompt_tokens) {
            ...
            while (remaining != 0) {
                ...
                if (staged.next_capture < staged.capture_groups.size()) {
                    ...
                    if (state_images->mtp_hidden_store() != nullptr) {
                        rewrite_capture_mtp_hidden =
                            state_images->mtp_hidden_slot(selectors.destination);
                        schedule_state.rewrite_checkpoint_mtp_hidden = &rewrite_capture_mtp_hidden;
                    }
                } else {
                    schedule_state.rewrite_checkpoint_hidden     = nullptr;
                    schedule_state.rewrite_checkpoint_mtp_hidden = nullptr;
                }
            ...
            copy_tail(sequence,
                      prefill_hidden.slice(1, static_cast<std::int32_t>(final_chunk_tokens) - 1, 1),
                      tail_mtp_hidden.data != nullptr ? &tail_mtp_hidden : nullptr);
        } else {
            ...
            if (staged.prepare_mtp) {
                ...
                schedule::mtp_bridge_and_propose(
                    schedule_state, io.token,
                    sequence.tail_mtp_hidden.data != nullptr ? sequence.tail_mtp_hidden
                                                             : sequence.tail_hidden,
        ...
        copy_round_token();
        std::array<TokenId, qwen3_8_flash_next::kMtpDecodeMaximumDrafts> initial_drafts{};
    ...
    try {
        ...
        for (std::size_t row = 0; row < lanes.size(); ++row) {
            ...
            if (!flash_ple_host || !flash_decode_ple) {
                throw std::logic_error("Flash-Next ordinary PLE staging is unavailable");
            }
            const std::size_t suffix_size = std::min<std::size_t>(3, sequence.ledger.size());
            std::vector<std::int32_t> suffix(suffix_size);
            std::copy(sequence.ledger.end() - static_cast<std::ptrdiff_t>(suffix_size),
...
MemorySummary ProgramImplCore::memory_summary() const noexcept {
    ...
    out.weights = ArenaMemorySummary{model.weight_bytes, model.weight_bytes, model.weight_bytes};
    out.sequence =
