...
PagedKVCacheLayout plan_cache(LayoutBuilder& builder, std::uint32_t layers, std::uint32_t capacity,
                              ...
                              const std::vector<KVPlaneGeometry>& auxiliary_planes) {
    if (layers == 0 ||
    ...
    geometry.planes.reserve(static_cast<std::size_t>(layers) *
                            (layer_storage.planes_per_layer() + auxiliary_planes.size()));
    for (std::uint32_t layer = 0; layer < layers; ++layer) {
        ...
        geometry.planes.insert(geometry.planes.end(), auxiliary_planes.begin(),
                               auxiliary_planes.end());
    }
    return PagedKVCacheLayout{
        ...
        .auxiliary_planes = auxiliary_planes,
    };
}
...
DecoderStateLayout plan_decoder_state(LayoutBuilder& builder, const DecoderStateSpec& spec) {
    ...
    layout.text_kv = plan_cache(builder, spec.full_attention_layers, spec.capacity, spec.kv_heads,
                                ...
                                spec.text_physical_page_groups, spec.auxiliary_planes);
    if (spec.enable_mtp) {
        layout.mtp_kv = plan_cache(builder, spec.mtp_layers, spec.capacity, spec.kv_heads,
                                   spec.attention_head_dim, spec.kv_storage, spec.kv_table_rows,
                                   spec.mtp_physical_page_groups, spec.auxiliary_planes);
    }
    return layout;
}
...
PagedKVCache::PagedKVCache(DeviceSpan backing, const PagedKVCacheLayout& layout)
    : pages_(backing, layout.pages), execution_tables_(backing, layout.execution_tables, pages_),
      ...
      layer_storage_(layout.layer_storage), auxiliary_planes_(layout.auxiliary_planes) {
    if (pages_.plane_count() !=
        static_cast<std::size_t>(layers_) *
            (layer_storage_.planes_per_layer() + auxiliary_planes_.size())) {
...
PagedKVLayerView PagedKVCache::layer_view(std::uint32_t layer, Tensor block_table) const {
    ...
    const std::size_t primary_planes = layer_storage_.planes_per_layer();
    const std::size_t stride         = primary_planes + auxiliary_planes_.size();
    ...
    auxiliary.reserve(auxiliary_planes_.size());
    for (std::size_t i = 0; i < auxiliary_planes_.size(); ++i) {
        auxiliary.push_back(pages_.plane(base + primary_planes + i));
    }
    return PagedKVLayerView{
        ...
        .auxiliary_pages = std::move(auxiliary),
...
PagedKVBatchLayerView PagedKVCache::batch_layer_view(std::uint32_t layer) const {
    ...
    return PagedKVBatchLayerView{
        ...
        .auxiliary_pages = direct.auxiliary_pages,
